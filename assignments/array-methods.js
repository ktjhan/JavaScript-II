...
runners.forEach(ele => fullNames.push(ele.first_name + " " + ele.last_name));
...
runners.map(ele => firstNamesAllCaps.push(ele.first_name.toUpperCase()));
...
runners.filter(ele => runnersLargeSizeShirt.push(ele.shirt_size === 'L'));
...
ticketPriceTotal = donations.reduce((sum, donation) => sum + donation);
...
// Problem 1
let lastNamesAndSize = [];

runners.forEach(ele => fullNames.push(ele.last_name + " " + ele.shirt_size));

console.log(lastNamesAndSize);


// Problem 2
let lastNamesAllCaps = [];

runners.map(ele => lastNamesAllCaps.push(ele.last_name.toUpperCase()));
...
runners.filter(ele => runnersMediumSizeShirt.push(ele.shirt_size === 'M'));
